...
const LeadDetailsPage = ({ lead, onClose, onLeadConverted,onLeadUpdated, userId, currentUser }) => {
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto transform transition-all">
        ...
        <div className="p-6">
          {activeTab1 === 'Overview' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <DetailItem label="Status" value={getStatus()}>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                    getStatus() === 'New' ? 'bg-blue-100 text-blue-800' :
                    getStatus() === 'Contacted' ? 'bg-purple-100 text-purple-800' :
                    'bg-green-100 text-green-800'
                  }`}>
                    {getStatus()}
                  </span>
                </DetailItem>
                <DetailItem label="Lead Source" value={lead.Lead_Source?.zc_display_value || 'N/A'} />
              ...
              <div className="space-y-4">
                ...
                <DetailItem label="Job Stack" value={lead.Stack.zc_display_value || 'N/A'} />
                <DetailItem label="No of Connects" value={lead.No_of_Connects || '0'} />
                <DetailItem
                  label="Job Link"
                  value={lead.Job_Link?.url || lead.Job_Link?.value || 'N/A'}
                  isLink
                />
                <DetailItem label="Select Profile" value={lead.Profile.zc_display_value || 'N/A'} />
              </div>
                ...
                <DetailItem
                  ...
                  value={
                    lead.Created_Time || lead.Added_Time
                      ? new Date(lead.Created_Time || lead.Added_Time).toLocaleString()
                      : 'N/A'
                  }
                />
              </div>
          ...
          {activeTab1 === 'Attachments' && leadId != null && (
            <Attachments module="Leads" RecordId={leadId} currentUser={currentUser}/>
          )}
          ...
          {activeTab1 === 'Reminder' && (
            <Reminder module="Leads" RecordId={leadId} currentUser={currentUser} comment={true} />
          )}
           {activeTab1 === 'Meeting' && (
            <Meeting module="Leads" RecordId={leadId} currentUser={currentUser} lead={lead} userId={userId} onMeetingCreated={handleMeetingCreated}/>
          )}
