...
const AddLeadForm = ({ onClose, userId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(`Error: ${error.message || 'Failed to create lead'}`);
      setFieldErrors({ form: error.message || 'Failed to create lead' });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 overflow-y-auto">
      <div className="bg-white border border-gray-300 rounded-lg shadow-lg w-full max-w-4xl p-4 my-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-xl font-bold text-[#f29d29]">Add New Lead</h2>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="md:col-span-2 flex justify-end mt-2">
            ...
            >
              {isSubmitting || isUploadingFiles ? 'Submitting...' : 'Submit Lead'}
            </button>
          </div>
        </form>
      </div>
