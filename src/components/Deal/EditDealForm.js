  ...
  useEffect(() => {
    if (deal) {
      setFormData({
        ...
        account: deal.Accounts?.ID || "",
        contact: deal.Contacts?.ID || "",
        source: deal.Sources?.ID || "",
        profile: deal.Select_Profile?.ID || "",
        amount: deal.Amount || "",
        budgetConfirmed:
          deal.Budget_Confirmed === true || deal.Budget_Confirmed === "true",
        discoveryCompleted:
          deal.Discovery_Completed === true ||
          deal.Discovery_Completed === "true",
        roiAnalysisCompleted:
          deal.ROI_Analysis_Completed === true ||
          deal.ROI_Analysis_Completed === "true",
        isConverted: deal.Is_Converted === true || deal.Is_Converted === "true",
      });
      ...
      for (const file of selectedFiles) {
        ...
        if (response.code === 3000) {
          ...
          const fileUploadConfig = {
            ...
            report_name: "All_Attachments_Dev",
            id: recordId,
            field_name: "File_upload", // Make sure this matches your field name in Zoho
            file: file,
          };
    ...
    try {
      ...
      if (response.code === 3000) {
        ...
        if (selectedFiles.length > 0) {
          await uploadAttachments(deal.ID);
        }
        ...
        if (onUpdateSuccess)
          onUpdateSuccess({
            ...
            ID: deal.ID,
            Accounts:
              accounts.find((a) => a.ID === formData.account) || deal.Accounts,
            Contacts:
              contacts.find((c) => c.ID === formData.contact) || deal.Contacts,
            Sources:
              sources.find((s) => s.ID === formData.source) || deal.Sources,
            Select_Profile:
              profiles.find((p) => p.ID === formData.profile) ||
              deal.Select_Profile,
          });
        onClose();
      } else {
      ...
      setSubmitError(error.message || "Failed to update deal");
      toast.error(`Error: ${error.message || "Failed to update deal"}`);
    } finally {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 overflow-y-auto">
      <div className="bg-gray-100 border border-gray-300 rounded-lg shadow-lg w-full max-w-4xl p-4 my-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-xl font-bold text-[#f29d29]">Edit Deal</h2>
          <button
            onClick={onClose}
        ...
        >
              ...
              {/* {existingAttachments.length > 0 && (
                <div className="space-y-2">
                  {existingAttachments.map((file) => (
                    <div key={file.ID} className="flex items-center justify-between p-2 border rounded bg-white">
                      <div className="flex items-center space-x-2">
                        {isImageFile(file.File_Name) ? (
                          <div className="w-8 h-8 rounded overflow-hidden bg-gray-100">
                            <img 
                              src={file.File_upload} 
                              alt="Preview" 
          ...
          <div className="md:col-span-2 flex justify-end space-x-3 mt-4">
            ...
            >
              {isSubmitting || isUploadingFiles ? "Updating..." : "Update Deal"}
            </button>
          </div>
        </form>
      </div>
